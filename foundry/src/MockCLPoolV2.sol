...
/// @title MockCLPoolV2
/// @notice Same liquidity math as MockCLPool, but `getSqrtRatioAtTick` is
///         the exact Uniswap V3 TickMath function, with constants verbatim
///         from v3-core. This is the predictor the exact-layer fork suite
///         compares live mint returns against, so its arithmetic must agree
///         with the deployed managers' to mint rounding.
contract MockCLPoolV2 is MockCLPool {
    /// @dev `MockCLPool(0)` initialises the parent state with `tick = ANCHOR_TICK`
    ///      and `sqrtPriceX96 = 0`; both are then overwritten with the
    ///      caller's tick and the exact sqrt price computed by the V2 override
    ///      of `getSqrtRatioAtTick`. The parent's initial assignment is
